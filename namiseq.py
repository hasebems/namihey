...
class BlockIndependentLoop(Block):
    ...
    class PartOperator:
        def __init__(self, blk, part_num):
            self.part = npt.Part(blk, part_num)
            self.maxMeasure = 0
            self.next_tick = 0
            self.nextLoopStartTime = 0      # 次回 loop 先頭の Start からの経過時間
            self.currentLoopStartTime = 0   # 現 loop 先頭の Start からの経過時間
    ...
    def _return_to_loop_top(self, op):
        ...
        op.maxMeasure = 0
        largest_tick = op.part.return_to_top(self.tick_for_one_measure) # <<Part>>
        while largest_tick > self.get_whole_tick(op):
            op.maxMeasure += 1
    ...
    def generate_event(self, ev_time):
        ...
        for op in self.part_operator:
            ...
            if current_tick > op.next_tick:
                ...
                if op.next_tick != nlib.INVALID_TICK:
